...
VERSION = "GUI_FULL_2026_07_02_GEAR_BREAKDOWN"
...
class ClassStatRow:
    ...
    @property
    def gear_atk(self) -> int:
        return self.total_atk - self.actor_atk

    @property
    def gear_def(self) -> int:
        return self.total_def - self.actor_def

    @property
    def gear_mat(self) -> int:
        return self.total_mat - self.actor_mat

    @property
    def gear_mdf(self) -> int:
        return self.total_mdf - self.actor_mdf
...
class PartyBaseline:
    ...
    def as_dict(self) -> dict[str, float | int]:
        return {
            ...
            "avg_actor_atk": self.avg_actor_atk,
            "avg_gear_atk": self.avg_gear_atk,
            "avg_actor_def": self.avg_actor_def,
            "avg_gear_def": self.avg_gear_def,
            "avg_actor_mat": self.avg_actor_mat,
            "avg_gear_mat": self.avg_gear_mat,
            "avg_actor_mdf": self.avg_actor_mdf,
            "avg_gear_mdf": self.avg_gear_mdf,
...
class EncounterBalancer:
    ...
    def baseline(
        ...
        avg_actor_atk = avg(r.actor_atk for r in martial_rows)
        avg_gear_atk = avg(r.gear_atk for r in martial_rows)
        avg_actor_def = avg(r.actor_def for r in rows)
        avg_gear_def = avg(r.gear_def for r in rows)
        avg_actor_mat = avg(r.actor_mat for r in caster_rows)
        avg_gear_mat = avg(r.gear_mat for r in caster_rows)
        avg_actor_mdf = avg(r.actor_mdf for r in rows)
        avg_gear_mdf = avg(r.gear_mdf for r in rows)
        ...
        return PartyBaseline(
            ...
            avg_mdf=avg_mdf,
            avg_agi=avg_agi,
            avg_actor_atk=avg_actor_atk,
            avg_gear_atk=avg_gear_atk,
            avg_actor_def=avg_actor_def,
            avg_gear_def=avg_gear_def,
            avg_actor_mat=avg_actor_mat,
            avg_gear_mat=avg_gear_mat,
            avg_actor_mdf=avg_actor_mdf,
            avg_gear_mdf=avg_gear_mdf,
...
def fmt_breakdown(raw: float | int, gear: float | int, total: float | int) -> str:
    return f"{fmt_num(raw)} + {fmt_num(gear)} = {fmt_num(total)}"
...
def print_baseline(baseline: PartyBaseline, show_classes: bool = True) -> None:
    ...
    print("=" * 72)
    print("Breakdowns use: level-only stat + expected gear contribution = expected total.")
    print("ATK/MAT gear is expected weapon budget; DEF/MDF gear is expected armor/shield budget.")
    print_kv_table([
        ("Avg MHP", fmt_num(baseline.avg_mhp)),
        ("Avg MMP", fmt_num(baseline.avg_mmp)),
        ("Avg ATK, martial Lv + gear = total", fmt_breakdown(baseline.avg_actor_atk, baseline.avg_gear_atk, baseline.avg_atk)),
        ("Avg DEF, Lv + gear = total", fmt_breakdown(baseline.avg_actor_def, baseline.avg_gear_def, baseline.avg_def)),
        ("Avg MAT, caster Lv + gear = total", fmt_breakdown(baseline.avg_actor_mat, baseline.avg_gear_mat, baseline.avg_mat)),
        ("Avg MDF, Lv + gear = total", fmt_breakdown(baseline.avg_actor_mdf, baseline.avg_gear_mdf, baseline.avg_mdf)),
    ...
    if show_classes:
        ...
        headers = [
            "Class", "MHP", "MMP",
            "ATK Lv", "ATK Gear", "ATK Total",
            "DEF Lv", "DEF Gear", "DEF Total",
            "MAT Lv", "MAT Gear", "MAT Total",
            "MDF Lv", "MDF Gear", "MDF Total",
            "AGI", "Hits",
        ]
        rows = []
...
def make_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--menu", action="store_true", help="Launch the terminal interactive menu.")
    parser.add_argument("--gui", action="store_true", help="Launch the Tkinter desktop GUI.")
    ...
    sub.add_parser("menu", help="Launch the terminal interactive menu.")
...
def run_export_baselines(args: argparse.Namespace) -> None:
    ...
    with output.open("w", newline="", encoding="utf-8") as f:
        fieldnames = [
            "level", "job_level", "avg_mhp", "avg_mmp",
            "avg_actor_atk", "avg_gear_atk", "avg_atk",
            "avg_actor_def", "avg_gear_def", "avg_def",
            "avg_actor_mat", "avg_gear_mat", "avg_mat",
            "avg_actor_mdf", "avg_gear_mdf", "avg_mdf",
            "avg_agi", "avg_martial_expected_hits",
            "avg_martial_basic_damage_vs_medium", "avg_caster_spell_damage_vs_medium",
        ]
...
def _capture_stdout(callback: Any, *args: Any, **kwargs: Any) -> str:
    import contextlib
    import io
...
class EncounterBalancerGUI:
    """Small desktop GUI wrapper around the balancing engine.

    Tkinter is part of the Python standard library, so this GUI does not add any
    external dependency. The calculation code remains shared with the CLI/menu.
    """

    def __init__(self, root: Any, args: argparse.Namespace):
        ...
        self.root = root
        self.args = args
        self.balancer: EncounterBalancer | None = None
        self.current_recommendation: EnemyRecommendation | None = None

        root.title("JRPG Encounter Balancer")
        root.geometry("1180x820")
        root.minsize(980, 680)

        self.data_var = tk.StringVar(value=str(getattr(args, "data", "") or ""))
        self.plugins_var = tk.StringVar(value=str(getattr(args, "plugins", "") or ""))
        self.def_share_var = tk.StringVar(value=str(getattr(args, "def_gear_share", 0.35)))
        self.mdf_share_var = tk.StringVar(value=str(getattr(args, "mdf_gear_share", 0.35)))
        self.status_var = tk.StringVar(value="Choose your data/plugins source, then click Load Project.")

        self._build_layout()
        if self.data_var.get().strip():
            root.after(100, self.load_project)
    ...
    def _build_layout(self) -> None:
        ...
        source = ttk.LabelFrame(outer, text="Project source", padding=8)
        source.pack(fill="x")
        source.columnconfigure(1, weight=1)
        source.columnconfigure(4, weight=1)

        ttk.Label(source, text="Data folder/ZIP").grid(row=0, column=0, sticky="w", padx=(0, 6), pady=3)
        ttk.Entry(source, textvariable=self.data_var).grid(row=0, column=1, sticky="ew", pady=3)
        ttk.Button(source, text="Browse file", command=lambda: self.browse_file(self.data_var)).grid(row=0, column=2, padx=3, pady=3)
        ttk.Button(source, text="Browse folder", command=lambda: self.browse_folder(self.data_var)).grid(row=0, column=3, padx=3, pady=3)

        ttk.Label(source, text="Plugins folder/ZIP").grid(row=1, column=0, sticky="w", padx=(0, 6), pady=3)
        ttk.Entry(source, textvariable=self.plugins_var).grid(row=1, column=1, sticky="ew", pady=3)
        ttk.Button(source, text="Browse file", command=lambda: self.browse_file(self.plugins_var)).grid(row=1, column=2, padx=3, pady=3)
        ttk.Button(source, text="Browse folder", command=lambda: self.browse_folder(self.plugins_var)).grid(row=1, column=3, padx=3, pady=3)

        ttk.Label(source, text="DEF gear share").grid(row=0, column=4, sticky="e", padx=(12, 6), pady=3)
        ttk.Entry(source, textvariable=self.def_share_var, width=8).grid(row=0, column=5, sticky="w", pady=3)
        ttk.Label(source, text="MDF gear share").grid(row=1, column=4, sticky="e", padx=(12, 6), pady=3)
        ttk.Entry(source, textvariable=self.mdf_share_var, width=8).grid(row=1, column=5, sticky="w", pady=3)
        ttk.Button(source, text="Load Project", command=self.load_project).grid(row=0, column=6, rowspan=2, padx=(12, 0), sticky="ns")

        ttk.Label(outer, textvariable=self.status_var).pack(fill="x", pady=(6, 8))
    ...
    def _new_tab(self, title: str) -> Any:
    ...
    def _make_output(self, parent: Any, row: int, column: int = 0, columnspan: int = 4, height: int = 22) -> Any:
        tk = self.tk
        ttk = self.ttk
        container = ttk.Frame(parent)
        container.grid(row=row, column=column, columnspan=columnspan, sticky="nsew", pady=(8, 0))
        parent.rowconfigure(row, weight=1)
        container.rowconfigure(0, weight=1)
        container.columnconfigure(0, weight=1)
        text = tk.Text(container, wrap="none", height=height, font=("Consolas", 10))
    ...
    def _put(self, widget: Any, content: str) -> None:
        widget.configure(state="normal")
        widget.delete("1.0", "end")
        widget.insert("1.0", content)
        widget.configure(state="normal")

    def _entry(self, parent: Any, label: str, variable: Any, row: int, col: int = 0, width: int = 12) -> None:
        self.ttk.Label(parent, text=label).grid(row=row, column=col, sticky="w", padx=(0, 6), pady=3)
        self.ttk.Entry(parent, textvariable=variable, width=width).grid(row=row, column=col + 1, sticky="w", pady=3)

    def _combo(self, parent: Any, label: str, variable: Any, values: Iterable[str], row: int, col: int = 0, width: int = 14) -> None:
        self.ttk.Label(parent, text=label).grid(row=row, column=col, sticky="w", padx=(0, 6), pady=3)
        combo = self.ttk.Combobox(parent, textvariable=variable, values=list(values), width=width, state="readonly")
        combo.grid(row=row, column=col + 1, sticky="w", pady=3)

    # ----- Source handling ------------------------------------------------

    def browse_file(self, var: Any) -> None:
        from tkinter import filedialog
        filename = filedialog.askopenfilename(filetypes=[("ZIP or all files", "*.zip *.*"), ("ZIP files", "*.zip"), ("All files", "*.*")])
        if filename:
            var.set(filename)

    def browse_folder(self, var: Any) -> None:
        from tkinter import filedialog
        folder = filedialog.askdirectory()
        if folder:
            var.set(folder)

    def source_args(self) -> argparse.Namespace:
        return argparse.Namespace(
            data=self.data_var.get().strip() or None,
            plugins=self.plugins_var.get().strip() or None,
            def_gear_share=float(self.def_share_var.get().strip() or 0.35),
            mdf_gear_share=float(self.mdf_share_var.get().strip() or 0.35),
        )

    def require_balancer(self) -> EncounterBalancer:
        if self.balancer is None:
            self.load_project()
        if self.balancer is None:
            raise ProjectFileError("Project is not loaded yet.")
    ...
    def load_project(self) -> None:
        ...
        try:
            ...
            self.balancer = build_balancer(args)
            self.args = _args_with(self.args, data=args.data, plugins=args.plugins, def_gear_share=args.def_gear_share, mdf_gear_share=args.mdf_gear_share)
            class_count = len(self.balancer.classes)
            enemy_count = len([e for e in self.balancer.db.enemies if e])
            self.status_var.set(f"Loaded project: {class_count} classes, {enemy_count} enemies. Data={args.data or 'default data folder'}, Plugins={args.plugins or 'default js/plugins folder'}")
    ...
    def get_int(self, var: Any, label: str, minimum: int | None = None, maximum: int | None = None) -> int:
    ...
    def get_float(self, var: Any, label: str, minimum: float | None = None, maximum: float | None = None) -> float:
    ...
    def get_optional_float(self, var: Any) -> float | None:
        raw = str(var.get()).strip()
        if not raw:
            return None
        return float(raw)

    def get_class_list(self, var: Any, fallback: list[str]) -> list[str]:
        return parse_class_list(str(var.get()).strip(), fallback)
    ...
    def _build_baseline_tab(self) -> None:
        tk = self.tk
        ttk = self.ttk
        tab = self._new_tab("Party baseline")
        self.base_level_var = tk.StringVar(value="25")
        self.base_job_level_var = tk.StringVar(value="25")
        self.base_martial_var = tk.StringVar(value=", ".join(DEFAULT_MARTIAL_CLASSES))
        self.base_caster_var = tk.StringVar(value=", ".join(DEFAULT_CASTER_CLASSES))
        self.base_show_classes_var = tk.BooleanVar(value=True)

        self._entry(tab, "Actor level", self.base_level_var, 0, 0)
        self._entry(tab, "Job level", self.base_job_level_var, 0, 2)
        ttk.Checkbutton(tab, text="Show class table", variable=self.base_show_classes_var).grid(row=0, column=4, sticky="w", padx=6)
        ttk.Label(tab, text="Martial classes").grid(row=1, column=0, sticky="w", pady=3)
        ttk.Entry(tab, textvariable=self.base_martial_var).grid(row=1, column=1, columnspan=4, sticky="ew", pady=3)
        ttk.Label(tab, text="Caster classes").grid(row=2, column=0, sticky="w", pady=3)
        ttk.Entry(tab, textvariable=self.base_caster_var).grid(row=2, column=1, columnspan=4, sticky="ew", pady=3)
        ttk.Button(tab, text="Calculate baseline", command=self.calculate_baseline).grid(row=3, column=0, pady=6, sticky="w")
        self.base_output = self._make_output(tab, row=99, columnspan=6)

    def calculate_baseline(self) -> None:
        try:
            balancer = self.require_balancer()
            level = self.get_int(self.base_level_var, "Actor level", 1)
            job_level = self.get_int(self.base_job_level_var, "Job level", 1, 99)
            martial = self.get_class_list(self.base_martial_var, DEFAULT_MARTIAL_CLASSES)
            caster = self.get_class_list(self.base_caster_var, DEFAULT_CASTER_CLASSES)
            baseline = balancer.baseline(level, job_level, martial, caster)
            output = _capture_stdout(print_baseline, baseline, self.base_show_classes_var.get())
            self._put(self.base_output, output)
        except Exception as exc:
            self.show_error(exc)
    ...
    def _build_recommend_tab(self) -> None:
        tk = self.tk
        ttk = self.ttk
        tab = self._new_tab("Enemy recommendation")

        self.rec_level_var = tk.StringVar(value="25")
        self.rec_job_level_var = tk.StringVar(value="25")
        self.rec_damage_pct_var = tk.StringVar(value="0.18")
        self.rec_attacker_var = tk.StringVar(value="physical")
        self.rec_durability_var = tk.StringVar(value="medium")
        self.rec_def_var = tk.StringVar(value="medium")
        self.rec_mdf_var = tk.StringVar(value="medium")
        self.rec_speed_var = tk.StringVar(value="average")
        self.rec_turns_var = tk.StringVar(value="4.0")
        self.rec_party_size_var = tk.StringVar(value="4")
        self.rec_phys_weight_var = tk.StringVar(value="0.60")
        self.rec_enemy_power_var = tk.StringVar(value="1.0")
        self.rec_player_magic_power_var = tk.StringVar(value="")
        self.rec_xp_share_var = tk.StringVar(value="1.0")
        self.rec_xp_scale_var = tk.StringVar(value="1.0")
        self.rec_martial_var = tk.StringVar(value=", ".join(DEFAULT_MARTIAL_CLASSES))
        self.rec_caster_var = tk.StringVar(value=", ".join(DEFAULT_CASTER_CLASSES))

        left = ttk.LabelFrame(tab, text="Core", padding=8)
        left.grid(row=0, column=0, sticky="nsew", padx=(0, 8), pady=(0, 6))
        right = ttk.LabelFrame(tab, text="Advanced", padding=8)
        right.grid(row=0, column=1, sticky="nsew", pady=(0, 6))
        tab.columnconfigure(0, weight=1)
        tab.columnconfigure(1, weight=1)

        self._entry(left, "Level", self.rec_level_var, 0, 0)
        self._entry(left, "Job level", self.rec_job_level_var, 0, 2)
        self._entry(left, "Damage % of avg MHP", self.rec_damage_pct_var, 1, 0)
        self._combo(left, "Attacker", self.rec_attacker_var, ["physical", "magical", "hybrid"], 1, 2)
        self._combo(left, "MHP durability", self.rec_durability_var, HP_PROFILE_MULTIPLIERS.keys(), 2, 0)
        self._combo(left, "DEF profile", self.rec_def_var, STAT_PROFILE_MULTIPLIERS.keys(), 2, 2)
        self._combo(left, "MDF profile", self.rec_mdf_var, STAT_PROFILE_MULTIPLIERS.keys(), 3, 0)
        self._combo(left, "Speed", self.rec_speed_var, SPEED_PROFILE_MULTIPLIERS.keys(), 3, 2)
        self._entry(left, "Turns to defeat", self.rec_turns_var, 4, 0)

        self._entry(right, "Party size", self.rec_party_size_var, 0, 0)
        self._entry(right, "Physical action weight", self.rec_phys_weight_var, 0, 2)
        self._entry(right, "Enemy attack power", self.rec_enemy_power_var, 1, 0)
        self._entry(right, "Player magic power", self.rec_player_magic_power_var, 1, 2)
        self._entry(right, "XP share", self.rec_xp_share_var, 2, 0)
        self._entry(right, "XP scale", self.rec_xp_scale_var, 2, 2)
        ttk.Label(right, text="Martial classes").grid(row=3, column=0, sticky="w", pady=3)
        ttk.Entry(right, textvariable=self.rec_martial_var).grid(row=3, column=1, columnspan=3, sticky="ew", pady=3)
        ttk.Label(right, text="Caster classes").grid(row=4, column=0, sticky="w", pady=3)
        ttk.Entry(right, textvariable=self.rec_caster_var).grid(row=4, column=1, columnspan=3, sticky="ew", pady=3)
        right.columnconfigure(3, weight=1)

        buttons = ttk.Frame(tab)
        buttons.grid(row=1, column=0, columnspan=2, sticky="w", pady=(0, 4))
        ttk.Button(buttons, text="Generate recommendation", command=self.calculate_recommendation).pack(side="left", padx=(0, 6))
        ttk.Button(buttons, text="Copy notetag", command=self.copy_current_notetag).pack(side="left", padx=6)
        ttk.Button(buttons, text="Copy params array", command=self.copy_current_params).pack(side="left", padx=6)

        self.rec_output = self._make_output(tab, row=99, columnspan=2)

    def calculate_recommendation(self) -> None:
        try:
            balancer = self.require_balancer()
            rec = balancer.recommend_enemy(
                level=self.get_int(self.rec_level_var, "Level", 1),
                job_level=self.get_int(self.rec_job_level_var, "Job level", 1, 99),
                damage_pct=self.get_float(self.rec_damage_pct_var, "Damage %", 0.0),
                attacker=str(self.rec_attacker_var.get()),  # type: ignore[arg-type]
                durability=str(self.rec_durability_var.get()),
                def_profile=str(self.rec_def_var.get()),
                mdf_profile=str(self.rec_mdf_var.get()),
                speed_profile=str(self.rec_speed_var.get()),
                turns_to_defeat=self.get_float(self.rec_turns_var, "Turns to defeat", 0.1),
                party_size=self.get_int(self.rec_party_size_var, "Party size", 1),
                physical_action_weight=self.get_float(self.rec_phys_weight_var, "Physical action weight", 0.0, 1.0),
                enemy_attack_power=self.get_float(self.rec_enemy_power_var, "Enemy attack power", 0.0),
                player_magic_power=self.get_optional_float(self.rec_player_magic_power_var),
                xp_share=self.get_float(self.rec_xp_share_var, "XP share", 0.0),
                xp_scale=self.get_float(self.rec_xp_scale_var, "XP scale", 0.0),
                martial_classes=self.get_class_list(self.rec_martial_var, DEFAULT_MARTIAL_CLASSES),
                caster_classes=self.get_class_list(self.rec_caster_var, DEFAULT_CASTER_CLASSES),
            )
            self.current_recommendation = rec
            self._put(self.rec_output, _capture_stdout(print_recommendation, rec))
        except Exception as exc:
            self.show_error(exc)

    def copy_current_notetag(self) -> None:
        from tkinter import messagebox
        if self.current_recommendation is None:
            messagebox.showinfo("Nothing to copy", "Generate a recommendation first.")
            return
        text = enemy_stats_notetag(self.current_recommendation.params, self.current_recommendation.level)
        self.root.clipboard_clear()
        self.root.clipboard_append(text)
        messagebox.showinfo("Copied", "Enemy true-stat notetag copied to clipboard.")
    ...
    def copy_current_params(self) -> None:
        ...
        if self.current_recommendation is None:
            messagebox.showinfo("Nothing to copy", "Generate a recommendation first.")
            return
        text = str(self.current_recommendation.rmmz_params_array())
        self.root.clipboard_clear()
        self.root.clipboard_append(text)
        messagebox.showinfo("Copied", "Fallback editor params array copied to clipboard.")
    ...
    def _build_exp_tab(self) -> None:
        tk = self.tk
        ttk = self.ttk
        tab = self._new_tab("EXP calculator")
        self.exp_level_var = tk.StringVar(value="25")
        self.exp_durability_var = tk.StringVar(value="medium")
        self.exp_def_var = tk.StringVar(value="medium")
        self.exp_mdf_var = tk.StringVar(value="medium")
        self.exp_speed_var = tk.StringVar(value="average")
        self.exp_turns_var = tk.StringVar(value="4.0")
        self.exp_share_var = tk.StringVar(value="1.0")
        self.exp_scale_var = tk.StringVar(value="1.0")

        self._entry(tab, "Level", self.exp_level_var, 0, 0)
        self._combo(tab, "MHP durability", self.exp_durability_var, HP_PROFILE_MULTIPLIERS.keys(), 0, 2)
        self._combo(tab, "DEF profile", self.exp_def_var, STAT_PROFILE_MULTIPLIERS.keys(), 1, 0)
        self._combo(tab, "MDF profile", self.exp_mdf_var, STAT_PROFILE_MULTIPLIERS.keys(), 1, 2)
        self._combo(tab, "Speed", self.exp_speed_var, SPEED_PROFILE_MULTIPLIERS.keys(), 2, 0)
        self._entry(tab, "Turns", self.exp_turns_var, 2, 2)
        self._entry(tab, "XP share", self.exp_share_var, 3, 0)
        self._entry(tab, "XP scale", self.exp_scale_var, 3, 2)
        ttk.Button(tab, text="Calculate EXP", command=self.calculate_exp).grid(row=4, column=0, pady=6, sticky="w")
        self.exp_output = self._make_output(tab, row=99, columnspan=5)

    def calculate_exp(self) -> None:
        try:
            balancer = self.require_balancer()
            output = _capture_stdout(
                print_exp_recommendation,
                balancer,
                self.get_int(self.exp_level_var, "Level", 1),
                str(self.exp_durability_var.get()),
                str(self.exp_def_var.get()),
                str(self.exp_mdf_var.get()),
                str(self.exp_speed_var.get()),
                self.get_float(self.exp_turns_var, "Turns", 0.1),
                self.get_float(self.exp_share_var, "XP share", 0.0),
                self.get_float(self.exp_scale_var, "XP scale", 0.0),
            )
            self._put(self.exp_output, output)
        except Exception as exc:
            self.show_error(exc)
    ...
    def _build_inspect_tab(self) -> None:
        tk = self.tk
        ttk = self.ttk
        tab = self._new_tab("Inspect enemy")
        self.enemy_combo_var = tk.StringVar(value="")
        self.inspect_level_var = tk.StringVar(value="")
        self.inspect_job_level_var = tk.StringVar(value="")
        self.enemy_combo = ttk.Combobox(tab, textvariable=self.enemy_combo_var, values=[], width=50, state="readonly")
        ttk.Label(tab, text="Enemy").grid(row=0, column=0, sticky="w", padx=(0, 6), pady=3)
        self.enemy_combo.grid(row=0, column=1, columnspan=4, sticky="ew", pady=3)
        self._entry(tab, "Level override", self.inspect_level_var, 1, 0)
        self._entry(tab, "Job level override", self.inspect_job_level_var, 1, 2)
        ttk.Button(tab, text="Refresh enemy list", command=self.refresh_enemy_list).grid(row=2, column=0, sticky="w", pady=6)
        ttk.Button(tab, text="Inspect enemy", command=self.inspect_enemy).grid(row=2, column=1, sticky="w", pady=6)
        self.inspect_output = self._make_output(tab, row=99, columnspan=5)

    def refresh_enemy_list(self) -> None:
        if not hasattr(self, "enemy_combo"):
            return
        values: list[str] = []
        if self.balancer is not None:
            for enemy in self.balancer.db.enemies:
                if enemy:
                    level = enemy_level_from_note(enemy.get("note", "")) or "?"
                    values.append(f"{enemy.get('id')}: {enemy.get('name')} (Lv {level})")
        self.enemy_combo.configure(values=values)
        if values and not self.enemy_combo_var.get():
            self.enemy_combo_var.set(values[0])

    def inspect_enemy(self) -> None:
        try:
            self.require_balancer()
            raw = self.enemy_combo_var.get().strip()
            match = re.match(r"(\d+)", raw)
            if not match:
                raise ValueError("Choose an enemy first.")
            level_raw = self.inspect_level_var.get().strip()
            job_raw = self.inspect_job_level_var.get().strip()
            ns = _args_with(
                self.source_args(),
                enemy_id=int(match.group(1)),
                level=int(level_raw) if level_raw else None,
                job_level=int(job_raw) if job_raw else None,
            )
            self._put(self.inspect_output, _capture_stdout(run_inspect_enemy, ns))
        except Exception as exc:
            self.show_error(exc)

    # ----- Export/reference tabs -----------------------------------------

    def _build_export_tab(self) -> None:
        tk = self.tk
        ttk = self.ttk
        tab = self._new_tab("Export baselines")
        self.export_path_var = tk.StringVar(value="jrpg_party_baselines.csv")
        self.export_levels_var = tk.StringVar(value="1,3,5,10,15,20,25,30,40,50,75,99,125,150,199,300,500,750,999")
        self.export_mode_var = tk.StringVar(value="cap99")
        ttk.Label(tab, text="Output CSV").grid(row=0, column=0, sticky="w", padx=(0, 6), pady=3)
        ttk.Entry(tab, textvariable=self.export_path_var).grid(row=0, column=1, columnspan=3, sticky="ew", pady=3)
        ttk.Button(tab, text="Browse", command=self.browse_export_csv).grid(row=0, column=4, sticky="w", padx=6)
        ttk.Label(tab, text="Levels").grid(row=1, column=0, sticky="w", padx=(0, 6), pady=3)
        ttk.Entry(tab, textvariable=self.export_levels_var).grid(row=1, column=1, columnspan=4, sticky="ew", pady=3)
        self._combo(tab, "Job level mode", self.export_mode_var, ["cap99", "same"], 2, 0)
        ttk.Button(tab, text="Export CSV", command=self.export_baselines).grid(row=3, column=0, sticky="w", pady=6)
        self.export_output = self._make_output(tab, row=99, columnspan=5)

    def browse_export_csv(self) -> None:
        from tkinter import filedialog
        filename = filedialog.asksaveasfilename(defaultextension=".csv", filetypes=[("CSV files", "*.csv"), ("All files", "*.*")])
        if filename:
            self.export_path_var.set(filename)

    def export_baselines(self) -> None:
        try:
            self.require_balancer()
            ns = _args_with(
                self.source_args(),
                output=self.export_path_var.get().strip(),
                levels=self.export_levels_var.get().strip(),
                job_level_mode=self.export_mode_var.get().strip(),
            )
            self._put(self.export_output, _capture_stdout(run_export_baselines, ns))
        except Exception as exc:
            self.show_error(exc)

    def _build_reference_tab(self) -> None:
        ttk = self.ttk
        tab = self._new_tab("Reference")
        ttk.Button(tab, text="Refresh reference", command=self.show_reference).grid(row=0, column=0, sticky="w", pady=6)
        self.reference_output = self._make_output(tab, row=99, columnspan=5)

    def show_reference(self) -> None:
        try:
            lines: list[str] = []
            lines.append("Durability / MHP profiles")
            lines.append("=" * 72)
            for k, v in HP_PROFILE_MULTIPLIERS.items():
                lines.append(f"{k:<14} {v}")
            lines.append("\nDEF/MDF profiles")
            lines.append("=" * 72)
            for k, v in STAT_PROFILE_MULTIPLIERS.items():
                lines.append(f"{k:<14} {v}")
            lines.append("\nSpeed profiles")
            lines.append("=" * 72)
            for k, v in SPEED_PROFILE_MULTIPLIERS.items():
                lines.append(f"{k:<14} {v}")
            if self.balancer is not None:
                lines.append("\nLoaded classes")
                lines.append("=" * 72)
                for cls in self.balancer.classes:
                    hits = ", ".join(f"Lv {lv}: {lo}-{hi}" for lv, lo, hi in cls.attack_hits) or "1-1"
                    lines.append(f"#{cls.class_id:<3} {cls.database_name:<24} formula={cls.job_name:<14} hits={hits}")
                lines.append("\nEXP pacing samples")
                lines.append("=" * 72)
                lines.append(f"{'Lv':>5}  {'XP to next':>12}  {'Battles/lv':>10}  {'Base encounter XP':>18}")
                for level in [1, 3, 5, 10, 25, 50, 75, 99, 125, 150, 199]:
                    if level <= self.balancer.exp_curve.max_level:
                        xp_next = self.balancer.exp_curve.xp_to_next_level(level)
                        battles = self.balancer.exp_curve.target_battles_per_level(level)
                        lines.append(f"{level:>5}  {xp_next:>12}  {battles:>10.2f}  {xp_next / max(1, battles):>18.2f}")
            self._put(self.reference_output, "\n".join(lines))
        except Exception as exc:
            self.show_error(exc)
...
def launch_gui(args: argparse.Namespace) -> None:
    try:
        ...
        raise ProjectFileError(f"Tkinter is not available in this Python installation: {exc}") from exc

    root = tk.Tk()
    EncounterBalancerGUI(root, args)
    root.mainloop()
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        elif args.gui or args.command in {None, "gui"}:
            launch_gui(args)
